...
public class Controller implements Initializable {
  ...
  @FXML public VBox clientSide;

  @FXML public VBox serverSide;
  ...
  @FXML private TextField clientPathField;

  @FXML private ListView<FileInfo> clientFilesList;

  @FXML private ListView<String> serverFilesList;

  @Override
  public void initialize(URL location, ResourceBundle resources) {
    currentFolder = Paths.get(File.listRoots()[0].getAbsolutePath());
  ...
  public void btnLoginOnAction(ActionEvent actionEvent) {
    ...
    if (login.isEmpty() || password.isEmpty()) {
      showMessage(
          Alert.AlertType.WARNING, "Warning", "The \"Login\" or \"Password\" fields is empty!");
      return;
    }
  ...
  private void initCallBack() {
    ...
    fileTransfer.setGetFileListCallBack(
        args -> {
          Platform.runLater(
              () -> {
    ...
    fileTransfer.setDownloadFileCallback(
        args -> {
          Platform.runLater(
              () -> {
    ...
    fileTransfer.setDeleteFileCallback(
        args -> {
  ...
  public void btnUploadOnAction(ActionEvent actionEvent) {
    ...
    ByteBuf buff =
        fileTransfer.requestUploadFile(
            clientFilesList.getFocusModel().getFocusedItem().getFileName());
    if (buff == null) {
      return;
    }
    ...
    new Thread(
            () -> {
              fileTransfer.sendFile(networkClient.getCurrentChannel());
            })
        .start();
  }
  ...
  public void btnDeleteCloudOnAction(ActionEvent actionEvent) {
    ...
    networkClient
        .getCurrentChannel()
        .writeAndFlush(fileTransfer.sendSomeMessage(file, Requests.DELETE_FILE));
  }
  ...
  public void btnDownloadFromServer(ActionEvent actionEvent) {
    ...
    networkClient
        .getCurrentChannel()
        .writeAndFlush(fileTransfer.sendSomeMessage(downLoadFile, Requests.DOWNLOAD_FILE));
  }
  ...
  public void btnRenameCloudOnAction(ActionEvent actionEvent) {
    ...
    if (newName != null) {
      networkClient
          .getCurrentChannel()
          .writeAndFlush(
              fileTransfer.sendSomeMessage(
                  serverFilesList.getFocusModel().getFocusedItem() + " " + newName,
                  Requests.RENAME_FILE));
    }
